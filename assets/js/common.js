...
function updateGroup(Url,action) {
    var createGroup=12;
    var name ='';  
    
    if(action!='update'){
      var d =   $('#contact_group').val(action);
      var d1 =   $('#contact_group').val(); 
    }
    ...
    //name =  ($('#contact_group').val()).replace(/^\s+|\s+$/gm,'');
     try {
        var checkValues = $('input[name=checkAll]:checked').map(function()
         ...
         if(checkValues.length==0){
             $('#error_msg_grp').html('Please select contact to upate group').css('color','red');
